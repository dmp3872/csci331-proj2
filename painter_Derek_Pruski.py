...
def evaluate(im: np.ndarray):
	...
	# select for colors with more blue and less red
	blue_score = 0
	for c in colors:
		# give points for high blue values
		if c[2] > 200:
			blue_score += 1
		# take points for high red values
		if c[0] > 150:
			blue_score -= 1

	# return blue points - red points + num of colors (color points)
	return blue_score + colors.shape[0]
